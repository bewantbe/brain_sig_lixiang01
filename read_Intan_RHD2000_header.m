function ret = read_Intan_RHD2000_loader(rhd_path, verbose)
...
% rhd_path: path to RHD2000 header file (info.rhd)
% ret     : header information and data

[path, file, ext] = fileparts(rhd_path);
file = [file, ext];
...
if ~exist('verbose', 'var') || isempty(verbose) || verbose ~= 0
  FPRINTF = @fprintf;
else
  FPRINTF = @(varargin) 0;
end

ret = [];  % return value goes here
...
evalin('caller', ['ret.(''' variable_name ''') = ' variable_name ';']);
